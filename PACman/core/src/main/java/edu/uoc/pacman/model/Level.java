...
public class Level {
    ...
    private void parse() throws LevelException {
        ...
        try (InputStreamReader streamReader = new InputStreamReader(inputStream, StandardCharsets.UTF_8); BufferedReader reader = new BufferedReader(streamReader)) {
            ...
            this.pacman = new Pacman(new Position(Integer.parseInt(pacmanInfo[0]), Integer.parseInt(pacmanInfo[1])), Direction.valueOf(pacmanInfo[2]), State.valueOf(pacmanInfo[3]), this);
            ...
            while ((line = getFirstNonEmptyLine(reader)) != null) {
                ...
                ghostList.add(GhostFactory.getGhostInstance(Integer.parseInt(ghostInfo[1]), Integer.parseInt(ghostInfo[2]), ghostInfo[0], Direction.valueOf(ghostInfo[3]), edu.uoc.pacman.model.entities.characters.ghosts.Behaviour.valueOf(ghostInfo[4]), this));
            }
    ...
    public MapItem getMapItem(Position position) throws NullPointerException {
        //TODO
        return this.getMapItem(position.getX(), position.getY());
    }
    ...
    public boolean isPathable(Position position) {
        //TODO
        for (MapItem item : mapItemList) {
            if (item.getPosition().equals(position) && !(item instanceof Wall)) {
                return true;
            }
        }
        return false;
    ...
    public boolean hasWon() {
        //TODO
        for (MapItem item : this.mapItemList) {
            if (item instanceof Scorable && item instanceof Pickable) {
                return false;
            }
        }
        return true;
